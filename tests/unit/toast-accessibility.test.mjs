...
test('cloud toast stylesheet is on every production serve path', () => {
  const serverEnvelope = readFileSync(new URL('../../server/app.mjs', import.meta.url), 'utf8');
  const serverRoutes = readFileSync(new URL('../../server/app_routes.mjs', import.meta.url), 'utf8');
  const serverApp = `${serverEnvelope}\n${serverRoutes}`;
