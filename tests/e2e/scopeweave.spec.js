...
test.describe('ScopeWeave Planner', () => {
  ...
  test('disables export and gantt actions when there are no tasks', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: 'CSV 내보내기' })).toHaveAttribute('aria-describedby', 'task-dependent-actions-help');
    await expect(page.locator('#task-dependent-actions-help')).toBeVisible();
    await expect(page.locator('#task-dependent-actions-help')).toContainText('작업을 추가하거나 CSV를 가져오세요');
    ...
    await expect(page.getByRole('button', { name: '간트차트보기' })).toHaveAttribute('aria-describedby', 'task-dependent-actions-help');
  });
