...
assert_strix_workflow_pr_trigger_hardened() {
	...
	assert_file_contains "$workflow_file" "branches: [main]" "strix workflow scans the protected main branch"
...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" "emit_file_prefix" "opencode review prompt evidence is byte-capped before NVIDIA NIM requests"
	...
	assert_file_contains "$workflow_file" 'actions/workflows/strix.yml/runs?event=pull_request_target&per_page=30' "opencode approval scopes supplemental workflow-run lookup to Strix pull_request_target runs"
	...
	assert_file_not_contains "$workflow_file" '--file "$OPENCODE_EVIDENCE_FILE"' "opencode review must not attach evidence content to NVIDIA NIM requests"
	...
	assert_file_not_contains "$workflow_file" "github-models/" "opencode review must not select GitHub Models provider ids"
	...
	assert_file_contains "$workflow_file" 'HEAD_SHA: ${{ github.event.pull_request.head.sha }}' "opencode evidence step passes HEAD_SHA to failed-check evidence collection"
	...
	assert_file_not_contains "$workflow_file" "MODEL: github-models/openai/gpt-5" "opencode review must not use GitHub Models GPT-5"
	...
	assert_file_contains "$opencode_config" '"model": "nvidia-nim/nvidia/llama-3.3-nemotron-super-49b-v1.5"' "opencode config defaults to NVIDIA NIM Nemotron Super"
	...
	assert_file_contains "$opencode_config" '"context": 131072' "opencode config uses the NVIDIA NIM 128k context window"
	assert_file_contains "$opencode_config" '"output": 8192' "opencode config uses the NVIDIA NIM 8k output window"
	assert_file_not_contains "$opencode_config" "github-models" "opencode config must not define a GitHub Models provider"
	assert_file_not_contains "$opencode_config" "STRIX_GITHUB_MODELS_TOKEN" "opencode config must not read STRIX_GITHUB_MODELS_TOKEN"
	assert_file_not_contains "$opencode_config" "COPILOT_GITHUB_TOKEN" "opencode config must not read COPILOT_GITHUB_TOKEN"
	assert_file_not_contains "$opencode_config" "openai/gpt-5" "opencode config must not define GitHub Models GPT-5"
...
FAIL: opencode review uses NVIDIA NIM Nemotron Super as the default model (missing 'MODEL: nvidia-nim/nvidia/llama-3.3-nemotron-super-49b-v1.5')
EOF
...
assert_opencode_failed_check_fallback_emits_each_strix_report() {
	...
	for line_number in $(seq 1 90); do
		printf '# email parser fixture line %s\n' "$line_number"
	done >"$fixture_repo/backend/services/email_parser.py"
	for line_number in $(seq 1 40); do
		printf '// prompt studio fixture line %s\n' "$line_number"
	done >"$fixture_repo/frontend/src/app/prompt-studio/page.tsx"
...
EOF
	...
	(
		...
		env -u GITHUB_EVENT_NAME -u GITHUB_EVENT_PATH -u STRIX_TEST_CHANGED_FILES_OVERRIDE -u STRIX_INPUT_FILE_ROOT \
			...
			STRIX_LLM_FILE="$strix_llm_file" \
			LLM_API_KEY_FILE="$llm_api_key_file" \
			STRIX_PROCESS_TIMEOUT_SECONDS="3" \
	...
	assert_file_contains "$output_log" "Strix run timed out after 3s." "timeout cleanup output"
	local _
...
EOF
	...
	(
		...
		env -u GITHUB_EVENT_NAME -u GITHUB_EVENT_PATH -u STRIX_TEST_CHANGED_FILES_OVERRIDE -u STRIX_INPUT_FILE_ROOT \
			...
			STRIX_LLM_FILE="$strix_llm_file" \
			LLM_API_KEY_FILE="$llm_api_key_file" \
			STRIX_PROCESS_TIMEOUT_SECONDS="8" \
