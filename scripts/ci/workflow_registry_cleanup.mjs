 ...
 * `--expected-sha`, and `GITHUB_TOKEN`. Every target is re-read immediately
 * before mutation, the protected branch is rechecked throughout the operation,
 * and every disable is verified through GitHub's workflow metadata endpoint.
 *
...
/**
 * Convert read-only audit evidence into the exact workflow IDs eligible for an
 * operator disable action.
 */
export function buildWorkflowCleanupPlan({ evidence, expectedSha }) {
  ...
  for (const item of evidence.classifications) {
    ...
    if (workflowIds.has(item.workflow_id)) throw new Error(`cleanup plan contains duplicate workflow id ${item.workflow_id}`);
    ...
    targets.push({ workflow_id: item.workflow_id, path: item.path });
  }
  targets.sort((left, right) => left.workflow_id - right.workflow_id);
  return {
...
export function parseCleanupArgs(argv, environment = process.env) {
  ...
  return { repository, branch, expectedSha, preservePaths, apply };
}
...
export async function applyWorkflowCleanupPlan({
  ...
  if (!Array.isArray(targets)) throw new TypeError('cleanup targets must be an array');
  ...
  // Preflight the whole plan before the first mutation so obvious identity drift
  // cannot leave a partially applied cleanup.
...
export async function cleanupWorkflowRegistry({
  ...
  const plan = buildWorkflowCleanupPlan({ evidence, expectedSha: boundSha });
  if (!apply) return { mode: 'dry_run', plan, evidence };
  if (!expectedSha) throw new Error('expected-sha is required for apply mode');
