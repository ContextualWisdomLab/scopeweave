...
GitHub's documented `WorkflowState` values are accepted exactly: `active`, `deleted`, `disabled_fork`, `disabled_inactivity`, and `disabled_manually`. An unfamiliar future state fails closed instead of being silently interpreted as inactive.
...
`GITHUB_TOKEN` is optional for public reads and is sent only in the Authorization header. Tokens and failed response bodies are never printed. Authorization failure, missing resources (including a missing protected workflow directory), malformed JSON, incomplete pagination, duplicated workflow IDs, unknown workflow states, an off-origin pagination link, or protected-branch movement all fail closed without a partial success payload. Transient 500, 502, 503, and 504 responses use bounded 100 ms then 200 ms timer-backed retries before failing closed.
...
Regression coverage includes complete multi-page enumeration; truncation detection; duplicate workflow-ID rejection; real timer-backed bounded 5xx retry with an injectable test seam; fail-closed 403/404 handling including a missing protected workflow directory; malformed JSON; branch movement; off-origin pagination; exact case-sensitive paths; path reuse without ID collapse; documented workflow-state validation; GitHub dynamic workflows; explicit active-PR preservation; canonical preserve-path validation; a legitimate present one-shot-like workflow; and CLI rejection of write-like arguments.
...
GitHub. (2026d). *Disabling and enabling a workflow*. GitHub Docs. Retrieved August 15, 2026, from https://docs.github.com/en/actions/how-tos/manage-workflow-runs/disable-and-enable-workflows
