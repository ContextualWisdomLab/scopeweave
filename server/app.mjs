...
app.post('/api/auth/login', async (c) => {
  ...
  // Sentinel: Prevent user enumeration via timing attack.
  // Evaluate the password hash unconditionally using a dynamically generated dummy hash if user is not found.
  // Also explicitly coerce candidate password to string to avoid JSON TypeError on verification.
  const dummyHash = '0'.repeat(32) + ':' + '0'.repeat(128);
  const targetHash = u ? u.password_hash : dummyHash;
  const safePassword = typeof password === 'string' ? password : '';
  const isValid = verifyPassword(safePassword, targetHash);
