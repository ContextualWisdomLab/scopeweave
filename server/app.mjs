...
app.post('/api/auth/login', async (c) => {
  ...
  // 🛡️ Sentinel Security Mitigation: Prevent user enumeration via timing attack
  // Always evaluate verifyPassword with a valid-length hash so execution time is
  // constant regardless of whether the user exists in the database.
  const dummyHash = '0'.repeat(32) + ':' + '0'.repeat(128);
  const validPassword = verifyPassword(passwordStr, u ? u.password_hash : dummyHash);
