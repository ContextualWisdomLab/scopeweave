...
// A static dummy hash for timing attack prevention.
...
app.post('/api/auth/login', async (c) => {
  ...
  const candPw = typeof password === 'string' ? password : '';
  const hashToVerify = u ? u.password_hash : DUMMY_HASH;
  const valid = verifyPassword(candPw, hashToVerify);

  if (!u || !valid || typeof password !== 'string') {
