...
app.use('*', async (c, next) => {
  ...
  try {
    ...
    if (!quietLogs) {
      ...
      console.log(JSON.stringify({ ts: new Date().toISOString(), method: c.env?.[GUARD_ACCOUNTING_METHOD] || c.req.method, path: redactRequestLogPath(c.req.path), status: s, ms: Date.now() - t }));
    }
