...
- Bounded hosted Stripe Checkout provider calls to one 15-second, no-retry
  ...
  exact HTTPS `checkout.stripe.com` URLs without credentials or non-standard
  ports while preserving Stripe-issued client fragments; and mapped provider
  failures to sanitized no-store 502 responses.
