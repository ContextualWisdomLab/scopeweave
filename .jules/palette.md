...
## 2026-07-26 - Make tooltip-only elements keyboard focusable
**Learning:** Important contextual information provided via `title` attributes on non-interactive elements (like status badges or summary cards) is completely inaccessible to keyboard-only users because these elements cannot receive focus.
**Action:** When adding `title` tooltips to visually important non-interactive elements, add `tabindex="0"` to make them focusable, and ensure they have a visible focus ring (`:focus-visible`) so keyboard users know where they are.
