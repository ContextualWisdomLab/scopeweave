...
## 2026-09-26 - Prevent redundant screen reader announcements for icon-only buttons
**Learning:** When creating icon-only buttons dynamically via JavaScript, simply setting `.textContent` (e.g., `button.textContent = '✕'`) causes screen readers to redundantly announce the visual character alongside the `aria-label`.
**Action:** Wrap the visual symbol in `<span aria-hidden="true">` using `.innerHTML` (e.g., `button.innerHTML = '<span aria-hidden="true">✕</span>'`) to hide the visual character from screen readers and rely exclusively on the `aria-label`.
