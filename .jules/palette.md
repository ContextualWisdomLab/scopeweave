...
## 2026-09-11 - Restore keyboard focus based on data attributes after synchronous DOM replacement
**Learning:** Relying solely on DOM element references (e.g. `document.activeElement`) to restore focus fails when the application re-renders and replaces those specific DOM nodes.
**Action:** When tracking focus before an action that triggers a re-render, extract identifying metadata like `taskId` and `action` from the active element. Then use this metadata along with `requestAnimationFrame` to dynamically query and focus the newly rendered element after the DOM update.
