...
## 2026-08-26 - Add keyboard focus for tooltip elements
**Learning:** Elements like div or span that have tooltips (e.g. title or aria-label attributes) remain inaccessible to keyboard users unless explicitly made focusable.
**Action:** Always add tabindex="0" and an appropriate ARIA role (e.g., role="note") to non-interactive tooltip containers so screen readers can announce them and keyboard users can trigger their focus state. Add `:focus-visible` styles so the focus indicator is visibly clear.
