...
## 2026-07-23 - Focus Management in Dynamic DOM Arrays
**Learning:** In applications like ScopeWeave that destroy and recreate table rows or DOM elements during rapid state updates (`renderAll()`), caching `document.activeElement` across renders silently breaks keyboard accessibility because the cached node becomes detached. Standard `.focus()` calls on detached nodes are ignored by browsers, severely impacting keyboard navigation (like hitting Esc to close a modal or editor row).
**Action:** Always extract unique identifying data properties (like `.id` or `dataset.taskId` and `dataset.action`) from `document.activeElement` *before* the render loop runs, then re-query the newly generated DOM node using those attributes to safely call `.focus()` inside a `requestAnimationFrame`.
