...
## 2026-09-08 - Replace native disabled with aria-disabled for form submit buttons
**Learning:** Setting native `disabled` on form submit buttons prevents them from receiving focus and swallows all events (including clicks). This means if the button is disabled due to form validation errors, keyboard users cannot focus it to read tooltips, and click handlers cannot fire to show helpful toast messages explaining why the submission is blocked. Furthermore, intercepting form submissions with custom feedback requires `form.noValidate = true` so the browser's native validation doesn't block the custom logic.
**Action:** Use `aria-disabled="true"` instead of `disabled` for form submit buttons when they need to remain focusable and provide interactive feedback. Handle the visual disabled state in CSS. When doing so, check for `aria-disabled` in the form's `submit` event handler (or the button's click handler) and provide an actionable toast notification explaining the error. Ensure the form has `noValidate = true` set if replacing native validation.
