...
## 2026-08-29 - Fixed click interception issue on mobile viewports
**Learning:** During UI interactions (e.g. click tests) in some Viewports, there might be overlapping elements intercepting clicks due to CSS. When converting native \`disabled\` to \`aria-disabled\`, make sure UI automated tests pass explicitly in target viewports. Force clicking in UI tests simulates successful keyboard interactions and circumvents CSS overlapping flakiness.
**Action:** Always include \`force: true\` on automated Playwright tests involving DOM elements that may be obscured across varying viewports to ensure click handling functions accurately verify event interception logic without layout interference.
