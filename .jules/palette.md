...
## 2025-02-18 - Toast Notification Accessibility
**Learning:** Toast notifications need `role="status"` and `aria-atomic="true"` to ensure screen readers announce the entire message reliably when it appears, not just the changed parts, and without interrupting the user.
**Action:** Always include `role="status"` and `aria-atomic="true"` alongside `aria-live="polite"` for non-interruptive dynamic notifications.
