...
## 2025-02-23 - Make tooltips accessible via keyboard
**Learning:** Tooltips on non-interactive elements (like `div` or `span`) are ignored by keyboard navigation unless they can receive focus.
**Action:** Always add `tabindex="0"` and ensure visible focus states (`:focus-visible`) for non-interactive elements that have `title` or `aria-label` attributes intended for user consumption.
