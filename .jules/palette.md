...
## 2025-01-28 - Add Confirmation Dialog for CSV Import
...
## 2025-01-28 - Prevent accidental data loss in inline editors
...
## 2025-01-28 - Restore Focus When Dismissing Editors
**Learning:** Dismissing an inline editor often triggers a full re-render that replaces DOM nodes, causing the element that originally triggered the editor (like an edit or add-child button) to become detached. When this happens, browsers reset focus to the document body, breaking keyboard navigation. Relying on a cached element reference (like `state.previousFocus`) to call `.focus()` will fail silently on detached nodes.
**Action:** Before triggering the full DOM re-render that dismisses an editor, cache unique identifying properties of the currently focused element (e.g., `id`, `data-action`, and its parent row's `data-task-id`). After the re-render, use `requestAnimationFrame` to query the new DOM for the equivalent newly created element and restore focus to it.
