...
## 2026-07-23 - Prevent CSV DDE injection via pipe-prefixed formulas in backend audit export
**Vulnerability:** The backend CSV export for audit logs failed to properly sanitize pipe-prefixed (`|`) formulas due to a missing whitespace bypass check.
**Learning:** Spreadsheet formula defenses must cover all command-style prefixes including `|` across all CSV export boundaries, both frontend and backend, while also accounting for leading whitespace bypasses (since spreadsheet software ignores them).
**Prevention:** Use `/^\s*[=+\-@|]/` for CSV formula sanitization instead of just `/^[=+\-@|]/` to prevent leading whitespace from bypassing the defense.
