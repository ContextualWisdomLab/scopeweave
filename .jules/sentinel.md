...
## 2024-09-08 - SSRF Vulnerability in Webhooks
**Vulnerability:** The webhook creation endpoint allowed users to specify internal and loopback IP addresses (like `127.0.0.1`, `169.254.169.254`, `0x7f000001`), exposing internal network services and metadata endpoints to Server-Side Request Forgery (SSRF) attacks via outgoing webhook deliveries.
**Learning:** Naive string matching or ignoring IP formats like hexadecimal or octal isn't enough to prevent SSRF. We must parse the URL using the Node.js native `URL` class (which normalizes various IP formats into standard notation) and then rigorously check the normalized hostname against a comprehensive `net.BlockList` of internal, loopback, and cloud metadata subnets for both IPv4 and IPv6.
**Prevention:** Always parse untrusted URLs with the `URL` constructor to normalize formats (handling bypasses like integer IPs), extract the hostname, strip IPv6 brackets, check if it's an IP address using `net.isIP`, and validate it against a strictly defined `net.BlockList` of restricted subnets before making any outgoing requests.
