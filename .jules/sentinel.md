...
## $(date +%Y-%m-%d) - Bump @hono/node-server to fix Path Traversal
**Vulnerability:** A Path Traversal vulnerability (GHSA-frvp-7c67-39w9) was identified in `@hono/node-server` via encoded backslash (`%5C`) in the `serve-static` middleware on Windows environments.
**Learning:** Outdated dependencies with known vulnerabilities (like path traversal in static file servers) can expose the host filesystem to attackers. Regular dependency scanning via tools like Trivy helps catch these issues early.
**Prevention:** Regularly update dependencies. Update vulnerable packages such as `@hono/node-server` to patched versions (e.g. `^1.19.14` to `^2.0.12` or later) via package manager updates (`pnpm update` or `npm install`), ensuring both `package.json` and lockfiles reflect the fixed version.
