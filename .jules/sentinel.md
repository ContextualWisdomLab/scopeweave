...
## 2026-09-25 - Fix Server-Side Request Forgery (SSRF) in webhooks
**Vulnerability:** The webhook delivery mechanism using `fetch` was vulnerable to SSRF. An attacker could provide a webhook URL resolving to internal loopback/private IPs (like `127.0.0.1` or `169.254.169.254`), allowing the server to make unauthorized outbound requests to internal services. Additionally, the `fetch` client automatically followed redirects, and failed requests were retried without re-validating the URL.
**Learning:** Checking only the URL hostname against an IP blocklist is insufficient due to DNS resolution discrepancies and dual-stack environment behavior. Furthermore, `node:dns/promises` resolution must explicitly check both `resolve4` and `resolve6` because an IPv4-mapped IPv6 address (e.g., `::ffff:127.0.0.1`) could bypass IPv4-only checks but route to IPv4 destinations via Node.js HTTP clients.
**Prevention:** Always implement SSRF protection for user-controlled outbound requests. Parse the URL, explicitly resolve both IPv4 and IPv6 records (or validate the IP if no DNS lookup is required), and ensure every resolved IP is validated against both IPv4 and IPv6 blocklists (`node:net.BlockList`). Crucially, halt any retry logic if an SSRF check fails, and configure the HTTP client to not follow redirects automatically (e.g., `redirect: 'error'`).
