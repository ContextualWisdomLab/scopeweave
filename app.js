...
async function connectJsonSync() {
  ...
  try {
    ...
    await writeJsonSyncHandle(candidate);
    state.jsonSyncHandle = candidate;
    renderAll();
...
async function writeJsonSyncHandle(handle) {
...
async function writeJsonSyncFile() {
  ...
  await writeJsonSyncHandle(state.jsonSyncHandle);
}
